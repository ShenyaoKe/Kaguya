...
RETRY_SPLIT:
	...
	for (int i = 0; i < 2 * np; ++i)
	{
		...
		if (edget > bound.pMin[axis] && edget < bound.pMax[axis])
		{
			...
			//std::cout << "cost at i:" << i << " is " << cost << std::endl;
			if (cost < bestCost)
			{
				bestCost = cost;
				bestAxis = axis;
	...
	/*for (int i = 0; i < 2 * np; ++i)
	{
	std::cout << edges[bestAxis][i].primNum << ": "
	<< edges[bestAxis][i].t << "; Start/End: " << edges[bestAxis][i].type << std::endl;
	}
	std::cout << "Split on axis " << axisChar[bestAxis] << " at " << tsplit << std::endl;
	std::cout << "Best offset: " << bestOffest << std::endl;
	std::cout << "Prims above: ";
	for (int i = 0; i < primsAbove.size(); ++i)
	{
	std::cout << primsAbove[i] << " ";
	}std::cout << std::endl;
	std::cout << "Prims below: ";
	for (int i = 0; i < primsBelow.size(); ++i)
	{
	std::cout << primsBelow[i] << " ";
	}std::cout << std::endl;
	std::cout << "////////////////////////" << std::endl;*/
