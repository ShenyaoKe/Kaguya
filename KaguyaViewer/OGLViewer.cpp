	...
	, box_mesh(nullptr), model_mesh(nullptr)
{
	...
	/*auto r2s = view_cam->RasterToScreen.getMat();
	auto r2c = view_cam->RasterToCamera.getMat();
	auto c2s = view_cam->CameraToScreen.getMat();
	auto newpos = r2c(Vector3D(640, 480, 0), 1);
	newpos.printInfo("Raster to Camera");
	newpos = c2s(newpos, 1);
	newpos.printInfo("Camera to Screen");
	auto combmat = c2s *r2c ;
	newpos = r2s(Vector3D(640, 480, 0), 1);
	newpos.printInfo("Raster to Screen");
*/
	cout << "raster to screen\n";
...
void OGLViewer::initializeGL()
{
	...
	model_shader = new GLSLProgram("model_vs.glsl", "model_fs.glsl");
	gate_shader = new GLSLProgram("gate_vs.glsl", "gate_fs.glsl");
