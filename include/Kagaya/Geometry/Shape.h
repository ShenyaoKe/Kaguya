...
const double reCE = 5e-8;//ray epsillon coefficiency
//////////////////////////////////////////////////////////////////////////
...
public:
	...
	virtual bool getDifferentialGeometry(const Ray& inRay, DifferentialGeometry *queryPoint, double *tHit, double *rayEpsilon) const;
...
public:
	geoSphere();
	geoSphere(const Vector3D& pos, const double& radius);
	~geoSphere();
...
public:
	geoTorus(){};
	geoTorus(const Vector3D& pos, const double& radius, const double& secRadius);
	~geoTorus();

	void setCenter(const Vector3D& pos);
	void setRadius(double radius);
	void setSecRadius(double secRadius);//set section radius
	bool getDifferentialGeometry(const Ray& inRay, DifferentialGeometry *queryPoint, double *tHit, double *rayEpsilon) const;
	bool isInside(const Vector3D& pPos) const;

private:
	Vector3D c;//center
	double r = 1, sr = 0.5;// radius and section radius
};
/************************************************************************/
...
public:
	geoEllipsoid();
	geoEllipsoid(const Vector3D& pos, const double& semiA, const double& semiB, const double& semiC);
	~geoEllipsoid();

	void setCenter(const Vector3D& pos);
	void setSemiAxes(const double& semiA, const double& semiB, const double& semiC);
	bool getDifferentialGeometry(const Ray& inRay, DifferentialGeometry *queryPoint, double *tHit, double *rayEpsilon) const;
	Vector3D getNormal(const Vector3D& pos) const;
...
private:
	Vector3D c;//center
	double sa, sb, sc;//semi-principal axes of length a, b, c
};
/************************************************************************/
...
public:
	geoParaboloid();
	geoParaboloid(const Vector3D& pos, const double& semiA, const double& semiB, const double& semiC, PARABOLOID_TYPE newType);
	~geoParaboloid();
...
public:
	geoHyperboloid();
	geoHyperboloid(const Vector3D& pos, const double& semiA, const double& semiB, const double& semiC, HYPERBOLOID_TYPE newType);
	~geoHyperboloid();
