...
bool Mesh::loadOBJ(const char* filename)
{
	...
	cout << "OBJ File " << filename << " Loding Time:" << (double)(end - start) / CLOCKS_PER_SEC << "s" << endl;//Timer
	return true;
}
...
bool Mesh::getDifferentialGeometry(const Ray& inRay, DifferentialGeometry *queryPoint, double *tHit, double *rayEpsilon) const
{
	return false;
}
...
void exportVBO(Mesh* tri_mesh, int &size,
	double* &vtx_array, double* &uv_array, double* &norm_array)
{
	double *verts, *texcoord(nullptr), *nms(nullptr);
	bool has_texcoord(false), has_normal(false);
	size = tri_mesh->fids.size();
	vtx_array = new double[size * 9];
	verts = vtx_array;
	if (tri_mesh->fids[0]->uv >= 0)
	{
		uv_array = new double[size * 6];
		texcoord = uv_array;
		has_texcoord = true;
	}
	else
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (has_texcoord)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				...
				*texcoord++ = cur_texcoord->x; *texcoord++ = cur_texcoord->y;
			}
		}
		if (has_normal)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				...
				*nms++ = cur_normal->x; *nms++ = cur_normal->y; *nms++ = cur_normal->z;
			}
		}
	}
...
bool Triangle::getDifferentialGeometry(const Ray& inRay, DifferentialGeometry *queryPoint, double *tHit, double *rayEpsilon) const
{
	Vector3D v1 = *p[1] - *p[0];
	Vector3D v2 = *p[2] - *p[0];
	...
	double rayT;//ray triangle DifferentialGeometry length
	double cosTheta = normal * inRay.getDir();
	...
	*rayEpsilon = reCE * *tHit;
	...
	double s = A1[maxIndex] / A[maxIndex], t = A2[maxIndex] / A[maxIndex];
...
void Triangle::getNormal(const DifferentialGeometry *queryPoint) const
{
	...
	{
		double du1 = uv[0]->x - uv[2]->x;
		double du2 = uv[1]->x - uv[2]->x;
		double dv1 = uv[0]->y - uv[2]->y;
		double dv2 = uv[1]->y - uv[2]->y;
		Vector3D dp1 = *p[0] - *p[2], dp2 = *p[1] - *p[2];
		double det = du1 * dv2 - dv1 * du2;//determinant
		if (det == 0)
		{
			//Do something
		}
...
/*
void exportVertices(Triangle* tri_mesh, double* buffer)
{
	buffer = 
	for (int i = 0; i < 3; i++)
	{
...
void exportTexCoords(Triangle* tri_mesh, double* buffer)
{
	for (int i = 0; i < 2; i++)
	{
...
void exportNormals(Triangle* tri_mesh, double* buffer)
{
	for (int i = 0; i < 3; i++)
	{
