...
namespace filter
{
	...
	ImageData* hsvOffset(const ImageData* src, double hue = 0.0, double saturation = 0.0, double value = 0.0);
	ImageData* hueSwitch(const ImageData* src, double oriHue = 0.0, double range = 0.0, double newHue = 0.0);
	...
	ImageData* threshold(const ImageData* src, double th = 0.5);
	ImageData* edgeDetect(const ImageData* src, double **lumaImg = nullptr, EdgeOperator opType = EdgeOperator::SOBEL);
	...
	ImageData* emboss(const ImageData* src, double theta, int radius);
	...
	ImageData* circlepix(const ImageData* src, int radius, double para);

	// Transform
	ImageData* translate(const ImageData* src, const Vector2D& translation, int& winWid, int& winHgt);
	ImageData* rotate(const ImageData* src, double theta, int& winWid, int& winHgt);
	...
	ImageData* inflateScale(const ImageData* src, double radius, int& winWid, int& winHgt);
