...
ImageData* filter::curveAdj(const ImageData* cvImg, const ImageData* src)
{
	...
	for (int j = 0; j < postImg.height; j++) {
		for (int i = 0; i < postImg.width; i++) {
			...
			cvP[index].y += postImg.pixels[i][j].b / (double)postImg.histRGB[2][index * step];
		}
	}
	for (int i = 0; i < lv; i++) {
		cvP[i].getInfo();
	...
	for (int i = 0; i < lv - 1; i++) {
		...
		double t3 = 0 - p[0] + 3 * p[1] - 3 * p[2] + p[3];
		double t2 = 2 * p[0] - 5 * p[1] + 4 * p[2] - 1 * p[3];
		double t1 = 0 - p[0] + p[2];
		double t0 = 2 * p[1];
		double tmp;
        
		for (int j = 1; j <= step; j++) {
			double t = j / static_cast<double>(step);
...
ImageData* filter::hsvOffset(const ImageData* src, double hue, double saturation, double value)
{
	int wdt = src->getWidth(), hgt = src->getHeight();
	ImageData* ret = new ImageData(wdt, hgt);
	if (hue > 180 || hue < -180) {
...
ImageData* filter::hueSwitch(const ImageData* src, double oriHue, double range, double newHue)
{
	int wdt = src->getWidth(), hgt = src->getHeight();
	ImageData* ret = new ImageData(wdt, hgt);
	...
	double hueOffset = newHue - oriHue;
...
ImageData* filter::equalize(const ImageData* src)
{
	...
	double avgStep = hgt * wdt / bitlen;
	double eqArry[256];// euqalized src->data
	double sumArray[256];// original src->data
	unsigned char *offsets[3];// original src->data shift to what color
    
	/*for (int i = 1; i < 4; i++)
	{
		eqArry[0] = avgStep;
		sumArray[0] = static_cast<double>(histRGB[i][0]);
		offsets[i - 1] = new unsigned char[256];
        
		for (int j = 1; j < 256; j++)
		{
			eqArry[j] = eqArry[j - 1] + avgStep;
			sumArray[j] = sumArray[j - 1] + static_cast<double>(histRGB[i][j]);
		}
...
ImageData* filter::equalizeLuma(const ImageData* src)
{
	...
	double avgStep = src->getHeight() * src->getWidth() / 255.0;
	double eqArry[256];// euqalized src->data
	double sumArray[256];// original src->data
	unsigned char offset[256];// original src->data shift to what color
    
	eqArry[0] = avgStep;
	sumArray[0] = static_cast<double>(histRGB[0][0]);
    
	for (int j = 1; j < 256; j++) {
		eqArry[j] = eqArry[j - 1] + avgStep;
		sumArray[j] = sumArray[j - 1] + static_cast<double>(histRGB[0][j]);
	}
...
ImageData* filter::posterize(const ImageData* src, int level)
{
	...
	double color_div = 1.0 / static_cast<double>(level - 1.0);
	double* post_val = new double[level];
	*post_val = 0;
...
ImageData* filter::threshold(const ImageData* src, double th)
{
	ImageData* ret = new ImageData(src->getWidth(), src->getHeight());
	for (int i = 0; i < src->getWidth(); i++)
	{
...
ImageData* filter::edgeDetect(const ImageData* src, double **lumaImg, EdgeOperator opType)
{
	...
	for (int i = 0; i < width; i++)
	{
		for (int j = 0; j < height; j++)
		{
			...
			double gs_val = sqrt(sqr(sumX) + sqr(sumY));
...
ImageData* filter::boxBlur(const ImageData* src, int radius)
{
	...
	ImageData* ret = new ImageData(width, height);
	int size = (radius << 1) + 1;
	double kernel_val = 1.0 / static_cast<double>(size);
...
ImageData* filter::bilateral(const ImageData* src, int radius)
{
	...
	int size = (radius << 1) + 1;
	int kSize = sqr(size);
	double** gKernel;//Spatial Weight
	double** sKernel;//Similarity Weight
	double sigmaD = radius / 3.0;// Sigma for gaussian, Can be adjusted
	double sqSigmaD = sigmaD * sigmaD;// SigmaD^2
	double sigmaR = 0.1;// Can be adjusted
	double sqSigmaR = sqr(sigmaR);// SigmaR^2
	double kSum = 0.0;
	...
	for (int i = 0; i < width; i++)
	{
		for (int j = 0; j < height; j++)
		{
			...
			ColorRGBA tmpColor;
			//double obSize = 0.0;
			double tmpDist;//Similarity Distance
			kSum = 0;
...
ImageData* filter::motionBlur(const ImageData* src, const ImageData* mvImg, int radius)
{
	...
	int size = 2 * radius + 1;
	double** kernel = new double*[size];
	double sigmaX = radius / 3.0;
	double sqSigmaX = sigmaX * sigmaX;
	double sigmaY = radius / 12.0;
	...
	for (int j = 0; j < src->getHeight(); j++) {
		for (int i = 0; i < src->getWidth(); i++) {
			...
			for (int m = -radius; m <= radius; m++) {
				for (int n = -radius; n <= radius; n++) {
					double kX = dir.dotMul(Vector2D(m, n));
					double kY = normalDir.dotMul(Vector2D(m, n));
...
ImageData* filter::emboss(const ImageData* src, double theta, int radius)
{
	ImageData* ret = new ImageData(src->getWidth(), src->getHeight());
	int size = radius * 2 + 1;
	double** kernel = new double*[size];
	//double gMax = 0, gMin = 0;
	//double* gsTmp = new double[width * src->getHeight()];
	double kMax = 0, kMin = 0;
	//double* yTmp = new double[width * src->getHeight()];
...
ImageData* filter::circlepix(const ImageData* src, int radius, double para)
{
	ImageData* ret = new ImageData(src->getWidth(), src->getHeight());
...
ImageData* filter::rotate(const ImageData* src, double theta, int& winWid, int& winHgt)
{
	ImageData* ret = new ImageData(src->getWidth(), src->getHeight());
...
ImageData* filter::bilnear(const ImageData* src, Vector2D* pos, int& winWid, int& winHgt)
{
	ImageData* ret = new ImageData(src->getWidth(), src->getHeight());
	// 
	double a0, a1, a2, a3, b0, b1, b2, b3, c0, c1, c2;
	double u, v;
	// 1, u, v, u*v
...
ImageData* filter::inflateScale(const ImageData* src, double radius, int& winWid, int& winHgt)
{
	ImageData* ret = new ImageData(src->getWidth(), src->getHeight());
	Matrix3D sMat, tMat1, tMat2, mat, iMat;
	tMat1.setTranslation(Vector2D(-width / 2.0, -height / 2.0));
	...
	for (int i = 0; i < winWid; i++)
	{
		for (int j = 0; j < winHgt; j++)
		{
			...
			{
				double sc = (radius - sqrt((radius - len) * radius)) / len;
				//error 01
				//double sc = (radius - sqrt((radius - len / (lenV.x / len + 2)) * radius) * (lenV.x / len + 2)) / len;
				//cout << sc << endl;
