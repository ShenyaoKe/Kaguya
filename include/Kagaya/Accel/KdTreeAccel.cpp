...
KdTreeAccel::KdTreeAccel(const vector<Shape*> &prims, int md, int mp,
	double eb) : maxDepth(md), maxPrims(mp), emptyBonus(eb)
{
	int np = prims.size();
	primitives = prims;
	...
	if (maxDepth <= 0)
	{
		maxDepth = Round2Int(8 + 1.3 * Log2Int(static_cast<double>(np)));
	}
	...
	int depth, BoundEdge *edges[3])
{
	...
	double oldCost = np;
	double totalSA = bound.surfaceArea();
...
RETRY_SPLIT:
	...
	for (int i = 0; i < 2 * np; ++i)
	{
		...
		if (edget > bound.pMin[axis] && edget < bound.pMax[axis])
		{
			...
			double belowSA = 2 * (bbDiag[axis0] * bbDiag[axis1] +
				(edget - bound.pMin[axis]) *
				(bbDiag[axis0] + bbDiag[axis1]));
			double aboveSA = 2 * (bbDiag[axis0] * bbDiag[axis1] +
			...
			double cost = (1.0 - eb) * (pBelow * nBelow + pAbove * nAbove);
	...
	double tsplit = edges[bestAxis][bestOffest].t;
	...
	double *tHit, double *rayEpsilon) const
{
	...
	double tmin, tmax, rayEp;//temprary DifferentialGeometry result
	...
	if (node != nullptr)
	{
		...
		else//if hit interior node
		{
			...
			int axis = node->flags;
			double tsplit = (node->split - inRay.pos[axis]) * invDir[axis];
...
void KdAccelNode::initInterior(int axis, double s)
{
	flags = axis;
	split = s;
