...
{
	double ramp = queryPoint->getDiffuseTheta();
...
PerlinNoiseTexture::PerlinNoiseTexture(double persistence, int octaves, int wd, int ht)
{
...
{
	double pValue = clampFromZeroToOne((noise.getValue(uv) - vmin) / (vmax - vmin));
...
PerlinNoiseTexture3D::PerlinNoiseTexture3D(double persistence, int octaves,
...
void PerlinNoiseTexture3D::setRange(const double& max_value, const double& min_value)
{
	vmin = min_value;
	vmax = max_value;
}
...
WoodTexture3D::WoodTexture3D(double persistence, int octaves, int size, int thr)
...
{
	double pValue = threshold * (noise.getValue(uv) - vmin) / (vmax - vmin);
